...
def execute(filters: Filters = None) -> tuple:
    from_date = filters["from_date"]  # Capture from_date
    to_date = filters["to_date"]      # Capture to_date
    filters.ranges = [num.strip() for num in filters.range.split(",") if num.strip().isdigit()]
    columns = get_columns(filters)

    item_details = FIFOSlots(filters).generate()
    data = format_report_data(filters, item_details, from_date, to_date)
...
def format_report_data(filters: Filters, item_details: dict, from_date: str, to_date: str) -> list[dict]:
    _func = itemgetter(1)
    data = []
    ...
    for _item, item_dict in item_details.items():
        ...
        average_age = get_average_age(fifo_queue, from_date, to_date)
        earliest_age = date_diff(to_date, fifo_queue[0][1])
        latest_age = date_diff(to_date, fifo_queue[-1][1])
       	range_values = get_range_age(filters, fifo_queue, from_date, to_date, item_dict)
...
def get_average_age(fifo_queue: list, from_date: str, to_date: str) -> float:
...
def get_range_age(filters, fifo_queue, from_date, to_date, item_dict):
	precision = cint(frappe.db.get_single_value("System Settings", "float_precision", cache=True))
	...
	def __get_stock_ledger_entries(self) -> Iterator[dict]:
		...
		sle_query = (
			...
			.where(
				...
				& (sle.posting_date >= self.filters.get("from_date"))  # Add from_date filter
